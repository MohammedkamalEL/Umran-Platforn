...
import React from 'react';
import { motion } from 'motion/react';
import { Shield, Zap, Globe, Smartphone, BarChart3, Users, Cpu, Lock, Download, CheckCircle2, ChevronLeft } from 'lucide-react';
import { cn } from '../lib/utils';
import { PITCH_FEATURES, generatePitchDeck } from '../lib/pitchdeck';
...
const FEATURES = PITCH_FEATURES.map(f => ({
  ...f,
  icon: f.id === 'gis' ? Globe : f.id === 'omni' ? Smartphone : f.id === 'ai' ? Cpu
      : f.id === 'transparency' ? Shield : f.id === 'stakeholders' ? Users : BarChart3,
  iconColor: f.id === 'gis' ? 'text-emerald-500' : f.id === 'omni' ? 'text-blue-500' : f.id === 'ai' ? 'text-purple-500'
      : f.id === 'transparency' ? 'text-amber-500' : f.id === 'stakeholders' ? 'text-rose-500' : 'text-indigo-500',
}));
...
export default function PlatformProfile({ onClose }: PlatformProfileProps) {
  return (
    ...
    >
      {/* Nav */}
      <nav className="sticky top-0 z-[120] bg-white/80 backdrop-blur-xl border-b border-slate-100 px-6 lg:px-12 py-4 flex items-center justify-between">
        <button
          onClick={onClose}
          className="flex items-center gap-2 text-slate-500 hover:text-emerald-600 font-bold transition-colors text-sm group"
        >
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex flex-col items-end">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">PLATFORM_VERSION</p>
            <p className="text-xs font-bold text-emerald-600">v2.4.0-SDN_PULSE</p>
          </div>
          <button
            onClick={generatePitchDeck}
            className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-wider hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-500/20 active:scale-95"
          >
            <Download size={14} />
      ...
      {/* Hero */}
      <section className="px-6 lg:px-12 pt-16 pb-20 bg-white relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-8 relative z-10">
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-50 text-emerald-700 text-xs font-black uppercase tracking-wider border border-emerald-100"
          >
            <Zap size={14} />
            بنية تحتية رقمية لجيل جديد
          </motion.div>
          ...
          <motion.h1
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
            className="text-5xl md:text-7xl lg:text-8xl font-black text-slate-900 tracking-tighter leading-tight font-display"
          >
            مواصفات منصة <span className="text-emerald-500">عُـمْـران</span>
          </motion.h1>
      ...
      <section className="px-6 lg:px-12 py-16 bg-slate-50">
        <div className="max-w-4xl mx-auto text-center">
          <motion.p
            ...
            viewport={{ once: true }}
            className="text-3xl md:text-5xl font-serif text-slate-900 leading-relaxed"
          >
            "مِنْ أَجْلِ <span className="text-emerald-600">سُـودَانٍ</span> يَنْهَضُ بِالْعِلْمِ وَيُبْنَى بِيَدِ أَبْنَائِه"
          </motion.p>
        </div>
      </section>
          ...
          {FEATURES.map((feature, i) => {
            ...
            return (
              <motion.div
                ...
                className="group p-8 bg-white border border-slate-100 rounded-3xl hover:border-emerald-200 hover:shadow-xl transition-all duration-500"
              >
                <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center mb-6 group-hover:bg-emerald-600 transition-all duration-500">
                  <Icon size={28} className={cn(feature.iconColor, "group-hover:text-white transition-colors duration-500")} />
                </div>
                <h3 className="text-lg font-black text-slate-900 mb-3">{feature.title}</h3>
                <p className="text-sm text-slate-500 leading-relaxed mb-4 border-r-2 border-slate-100 pr-3 group-hover:border-emerald-400 transition-colors">
                  {feature.description}
                </p>
                <div className="flex items-center justify-between bg-slate-50 rounded-xl p-3">
                  <CheckCircle2 size={14} className="text-emerald-500" />
                  <span className="text-[10px] font-black text-emerald-600 uppercase tracking-wider">{feature.stats}</span>
                </div>
              </motion.div>
            );
          })}
      ...
      {/* Security Section */}
      <section className="px-6 lg:px-12 py-16 bg-slate-50">
        <div className="max-w-6xl mx-auto bg-white rounded-3xl p-8 lg:p-16 border border-slate-100 shadow-sm">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center" dir="rtl">
            <div className="space-y-6">
              <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-black uppercase tracking-wider">
                SECURITY_CORE
              </span>
              <h2 className="text-3xl lg:text-4xl font-black text-slate-900 leading-tight">
                نزاهة البيانات <span className="text-emerald-600">فوق كل اعتبار</span>
              </h2>
              <p className="text-slate-500 leading-relaxed text-sm lg:text-base">
                نستخدم أحدث تقنيات التشفير والمصادقة لضمان أن كل بلاغ، مساهمة، أو عملية ميدانية موثقة ومؤمنة تماماً.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                  <Lock className="text-emerald-500 mb-3" size={20} />
                  <p className="text-sm font-black text-slate-900 mb-1">تشفير AES-256</p>
                  <p className="text-xs text-slate-400 leading-relaxed">حماية شاملة لكافة قواعد البيانات.</p>
                </div>
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100">
                  <Shield className="text-emerald-500 mb-3" size={20} />
                  <p className="text-sm font-black text-slate-900 mb-1">مصادقة ثنائية</p>
                  <p className="text-xs text-slate-400 leading-relaxed">ربط الوصول بنظام الهوية الرقمية.</p>
                </div>
              </div>
            </div>
            <div className="w-full aspect-square bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-center relative overflow-hidden">
              <div className="absolute inset-0 flex items-center justify-center">
                <motion.div
                  animate={{ rotate: 360 }}
                  transition={{ repeat: Infinity, duration: 20, ease: "linear" }}
                  className="w-[80%] h-[80%] border border-slate-200/50 rounded-full"
                />
                <motion.div
                  animate={{ rotate: -360 }}
                  transition={{ repeat: Infinity, duration: 12, ease: "linear" }}
                  className="w-[55%] h-[55%] border border-emerald-500/15 rounded-full absolute"
                />
              </div>
              <Cpu size={80} strokeWidth={1} className="text-emerald-600 animate-pulse relative z-10" />
              <div className="absolute bottom-6 inset-x-6 p-4 bg-white border border-slate-100 shadow-sm rounded-2xl text-right">
                <p className="text-[9px] font-black text-emerald-600 uppercase tracking-wider mb-1">NODE_STATUS: ACTIVE</p>
                <p className="text-xs font-bold text-slate-700">فحص أكثر من ٥٠٠٠ نقطة بيانات في الثانية.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="px-6 lg:px-12 py-12 border-t border-slate-100 text-center">
        <h2 className="text-3xl font-black text-slate-900 mb-2">عُـمْـران<span className="text-emerald-500">.</span></h2>
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">BUILDING_SUDAN_TOGETHER // ٢٠٢٦</p>
        <div className="flex justify-center gap-6">
          {['سياسة الخصوصية', 'شروط الاستخدام', 'تواصل معنا'].map(link => (
            <button key={link} className="text-xs font-bold text-slate-500 hover:text-emerald-600 transition-colors">{link}</button>
          ))}
        </div>
      </footer>
    </motion.div>
