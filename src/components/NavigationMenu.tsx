...
import { 
  Map as MapIcon, Users, Activity, User as UserIcon,
  LogIn, Settings, Shield, Search, Bell, Command, HelpCircle, X, Gavel
} from 'lucide-react';
import { cn } from '../lib/utils';
...
interface NavigationMenuProps {
  ...
  onClose: () => void;
  activeView: string;
  onNavigate: (view: 'dashboard' | 'campaigns' | 'stats' | 'profile' | 'report' | 'ministry' | 'tenders') => void;
  unreadCount?: number;
}

const MENU_ITEMS: { id: 'dashboard' | 'campaigns' | 'stats' | 'profile' | 'report' | 'ministry' | 'tenders'; label: string; sublabel: string; icon: React.ReactNode; badge?: string }[] = [
  { id: 'dashboard', label: 'المخطط القومي الشامل', sublabel: 'NATIONAL_GIS_SYNC', icon: <MapIcon size={20} /> },
  { id: 'campaigns', label: 'الحملات والمبادرات', sublabel: 'COMMUNITY_ACTION', icon: <Users size={20} /> },
  { id: 'stats', label: 'مركز تحليل البيانات', sublabel: 'ANALYTICS_CORE', icon: <Activity size={20} /> },
                ...
                >
                  <div className="flex items-center gap-2">
                    {item.badge && activeView !== item.id && (
                      <span className="text-[8px] font-black px-1.5 py-0.5 bg-emerald-500 text-white rounded-md uppercase tracking-wide">
                        {item.badge}
                      </span>
                    )}
                    <div className={cn(
