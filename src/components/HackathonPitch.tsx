...
import React from 'react';
import { motion } from 'motion/react';
import { Award, Target, TrendingUp, Shield, Signal, X, Star, Globe, Cpu } from 'lucide-react';
import { cn } from '../lib/utils';

export default function HackathonPitch({ onClose }: { onClose: () => void }) {
  const features = [
    { title: 'الذكاء الاصطناعي السيادي', desc: 'تحليل فوري للصور والأصوات لتصنيف الأعطال بدقة ٩٤٪ عبر محركات Gemini AI.', icon: Cpu, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { title: 'الجسر الرقمي SMS/USSD', desc: 'تكامل مع شبكات الاتصال الوطنية لضمان وصول البلاغات حتى بدون إنترنت.', icon: Signal, color: 'text-blue-600', bg: 'bg-blue-50' },
    { title: 'نظام النقاط والمكافآت', desc: 'تحفيز المشاركة المجتمعية عبر نظام تصنيف إقليمي يحول المتطوعين إلى سفراء إعمار.', icon: Star, color: 'text-amber-600', bg: 'bg-amber-50' },
    { title: 'الشفافية التشغيلية SLA', desc: 'تتبع لحظي للمهمات مع كود متابعة موحد يربط المواطن بغرفة العمليات مباشرة.', icon: Shield, color: 'text-rose-600', bg: 'bg-rose-50' },
  ];

  const businessModel = [
    { label: 'B2G Analytics', value: 'بيع بيانات تحليل الفجوات لوزارات البنية التحتية.' },
    { label: 'NGO Data Hub', value: 'ترخيص الوصول للوضع الميداني للمنظمات الدولية.' },
    { label: 'Utility Micro-Fees', value: 'رسوم رمزية على عمليات الدفع الإلكتروني.' },
    { label: 'Sustainable Ads', value: 'مساحات إعلانية للشركات المساهمة في الإعمار.' },
  ];

  return (
    <div className="p-6 lg:p-12 h-full flex flex-col bg-white overflow-auto" dir="rtl">
      {/* Header */}
      <div className="flex items-start justify-between mb-8">
        ...
        <div className="text-right">
          <div className="flex items-center gap-2 justify-end mb-2">
            ...
            <span className="text-xs font-black text-emerald-600 uppercase tracking-wider font-mono">رؤية الفوز // ٢٠٢٦</span>
          </div>
          <h2 className="text-3xl lg:text-5xl font-black tracking-tight leading-tight font-display text-slate-900">
            لماذا "عمران" هو المنصة الفائزة؟
          </h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
        ...
        <div className="bg-slate-50 rounded-3xl p-6 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-lg font-black text-slate-900">المميزات التنافسية</h3>
          </div>
          {features.map((f, i) => (
            <div key={i} className="flex gap-4 p-4 bg-white rounded-2xl border border-slate-100 hover:border-emerald-100 transition-colors group">
              <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform", f.bg, f.color)}>
                <f.icon size={18} />
              </div>
              <div>
        ...
        <div className="space-y-6">
          {/* Business Model */}
          <div className="bg-emerald-700 rounded-3xl p-6 text-white space-y-4">
            <div className="flex items-center gap-2">
              <TrendingUp size={18} />
              <h3 className="text-base font-black">استدامة النموذج المادي</h3>
            </div>
            <p className="text-xs text-emerald-100 leading-relaxed">عمران بنية تحتية اقتصادية رقمية تمول نفسها عبر أربعة روافد:</p>
            <div className="space-y-2">
              {businessModel.map((m, i) => (
                <div key={i} className="p-3 bg-white/10 rounded-xl border border-white/10 flex items-start gap-3">
                  <span className="w-6 h-6 rounded-lg bg-white/20 flex items-center justify-center text-[10px] font-black shrink-0">{i+1}</span>
                  <div>
                    <p className="text-[10px] font-black text-emerald-200 uppercase tracking-wider">{m.label}</p>
          ...
          {/* Global Vision */}
          <div className="bg-slate-50 rounded-3xl p-6 text-center space-y-4 border border-slate-100">
            <Globe className="text-blue-500 mx-auto" size={40} />
            <h3 className="text-xl font-black text-slate-900">الرؤية العالمية للسودان</h3>
            <p className="text-sm text-slate-500 leading-relaxed">
              نبني أكبر قاعدة بيانات ميدانية حية في المنطقة، مما يجعل السودان مركزاً إقليمياً لابتكارات المدن الذكية في بيئات ما بعد النزاعات.
            </p>
            <div className="flex justify-center gap-3">
              <span className="px-4 py-1.5 rounded-full border border-slate-200 text-[10px] font-black text-blue-600">SCALABLE</span>
              <span className="px-4 py-1.5 rounded-full border border-slate-200 text-[10px] font-black text-emerald-600">IMPACT_DRIVEN</span>
            </div>
          </div>
        </div>
      </div>
