...
export default function IssueDetail({ issue: initialIssue, onClose }: { issue: Issue; onClose: () => void }) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'بلاغ عمران السودان', text: shareText, url: window.location.href });
    ...
    async function fetchProfile() {
      if (auth.currentUser) {
        ...
        if (snap.exists()) {
          ...
          setUserProfile({ uid: auth.currentUser.uid, role: 'official', displayName: 'مسؤول النظام', isAnonymous: false });
        }
      }
    }
    fetchProfile();
  ...
  useEffect(() => {
    const unsubIssue = onSnapshot(doc(db, 'issues', initialIssue.id), (snapshot) => {
      if (snapshot.exists()) setLocalIssue({ id: snapshot.id, ...snapshot.data() } as Issue);
    }, (error) => console.error("Error fetching issue live data:", error));

    const qMove = query(collection(db, 'issues', initialIssue.id, 'movements'), orderBy('timestamp', 'desc'));
    const unsubMove = onSnapshot(qMove, (snapshot) => {
      setMovements(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Movement[]);
    }, (error) => console.error("Error fetching movements:", error));

    const qComm = query(collection(db, 'issues', initialIssue.id, 'comments'), orderBy('createdAt', 'desc'));
    const unsubComm = onSnapshot(qComm, (snapshot) => {
      setComments(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Comment[]);
    }, (error) => console.error("Error fetching comments:", error));

    return () => { unsubIssue(); unsubMove(); unsubComm(); };
  }, [initialIssue.id]);

  const handleUpvote = async () => {
    try { await updateDoc(doc(db, 'issues', localIssue.id), { reportCount: increment(1) }); }
  ...
  const handleAddComment = async (text: string) => {
    ...
    setSubmitting(true);
    try {
      await addDoc(collection(db, 'issues', localIssue.id, 'comments'), { text, userId: auth.currentUser.uid, createdAt: serverTimestamp() });
      setNewComment('');
    } catch (e) { console.error(e); }
  ...
  const handleStatusUpdate = async (status: Issue['status'], severity?: number) => {
    try {
      const stageMap: Record<string, Issue['currentStage']> = {
        'pending': 'detected', 'verified': 'sorting', 'in-progress': 'processing',
        'completed': 'verification', 'resolved': 'resolution'
      };
      const updateData: any = { status, currentStage: stageMap[status] || localIssue.currentStage, updatedAt: serverTimestamp() };
      if (severity) updateData.severity = severity;
      await updateDoc(doc(db, 'issues', localIssue.id), updateData);
      await addDoc(collection(db, 'issues', localIssue.id, 'movements'), {
        status: status === 'verified' ? 'تحقق ومراجعة' : status === 'in-progress' ? 'بدء المعالجة' : status === 'completed' ? 'تم التنفيذ - بانتظار الاعتماد' : 'تم حل البلاغ نهائياً',
  ...
  const handleResolve = async () => {
    if (!auth.currentUser || auth.currentUser.uid !== localIssue.reporterId) return;
    try {
      await updateDoc(doc(db, 'issues', localIssue.id), { status: 'resolved', currentStage: 'resolution', citizenSignOff: true, updatedAt: serverTimestamp() });
      await addDoc(collection(db, 'issues', localIssue.id, 'movements'), {
        status: 'إغلاق البلاغ', description: 'تم اعتماد الحل من قبل المواطن وإغلاق الملف بنجاح.',
        institutionName: 'المواطن | صاحب البلاغ', timestamp: Date.now(), stage: 'resolution'
      });
    } catch (e) { console.error(e); }
  };
  ...
  const logMovement = async (status: string, description: string) => {
    try {
      await addDoc(collection(db, 'issues', localIssue.id, 'movements'), {
        status, description, institutionName: localIssue.assignedInstitution || 'المؤسسة المسؤولة', timestamp: serverTimestamp()
      });
    } catch (e) { console.error(e); }
  };
  ...
  const assignInstitution = async (name: string) => {
    try {
      await updateDoc(doc(db, 'issues', localIssue.id), { assignedInstitution: name, status: 'verified', currentStage: 'sorting' });
      await addDoc(collection(db, 'issues', localIssue.id, 'movements'), {
        status: 'جهة مسؤولة: تم التعيين',
        description: `تم تعيين ${name} رسمياً كمؤسسة مسؤولة عن معالجة هذا البلاغ في النظام.`,
        institutionName: name, timestamp: Date.now(), stage: 'sorting'
      });
    } catch (e) { console.error(e); }
  };

  const voiceTools = [
    { name: 'upvote_issue', description: 'Supports or upvotes the current infrastructure issue being viewed.', parameters: { type: 'object', properties: {} } },
    { name: 'add_comment', description: 'Adds a comment or feedback to the current infrastructure issue.', parameters: { type: 'object', properties: { text: { type: 'string', description: 'The content of the comment in Arabic or Sudanese dialect.' } }, required: ['text'] } },
    { name: 'update_status', description: 'Updates the status of the issue. ONLY for officials.', parameters: { type: 'object', properties: { status: { type: 'string', enum: ['verified', 'in-progress', 'completed'] }, severity: { type: 'string' } }, required: ['status'] } },
    { name: 'log_movement', description: 'Logs a movement or action taken by the institution.', parameters: { type: 'object', properties: { status: { type: 'string' }, description: { type: 'string' } }, required: ['status', 'description'] } },
    { name: 'assign_institution', description: 'Assigns the issue to a specific government institution.', parameters: { type: 'object', properties: { name: { type: 'string', enum: INSTITUTIONS.map(i => i.name) } }, required: ['name'] } },
    { name: 'set_eta', description: 'Sets the expected resolution date for the issue. ONLY for officials.', parameters: { type: 'object', properties: { days: { type: 'number' } }, required: ['days'] } }
  ];

  const handleToolCall = async (call: any) => {
    if (call.name === 'upvote_issue') { await handleUpvote(); return { success: true, message: "تم دعم البلاغ بنجاح" }; }
    if (call.name === 'add_comment') { await handleAddComment(call.args.text); return { success: true, message: "تمت إضافة تعليقك" }; }
    if (call.name === 'update_status') {
      if (userProfile?.role === 'official' || userProfile?.role === 'partner') {
        const severityInt = call.args.severity ? parseInt(call.args.severity) : undefined;
        await handleStatusUpdate(call.args.status, severityInt);
        return { success: true, message: `تم تحديث حالة البلاغ إلى: ${call.args.status}` };
      }
      return { success: false, message: "عذراً، ليس لديك صلاحية لهذا الإجراء." };
    }
    if (call.name === 'log_movement') {
    ...
    if (call.name === 'assign_institution') {
      if (userProfile?.role === 'official') { await assignInstitution(call.args.name); return { success: true, message: `تم تعيين ${call.args.name}` }; }
      return { success: false, message: "صلاحيات غير كافية" };
    }
  };

  const statusLabel = localIssue.status === 'pending' ? 'قيد المراجعة' : localIssue.status === 'verified' ? 'تم التحقق' : localIssue.status === 'in-progress' ? 'جاري التنفيذ' : localIssue.status === 'completed' ? 'تم الإنجاز' : 'مغلق';
  const typeLabel = localIssue.type === 'road' ? 'طرق' : localIssue.type === 'water' ? 'مياه' : localIssue.type === 'electricity' ? 'كهرباء' : localIssue.type === 'waste' ? 'نفايات' : 'عام';
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-100 sticky top-0 z-[60] shadow-sm">
        <button
          onClick={onClose}
          className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 hover:bg-emerald-50 text-slate-600 hover:text-emerald-600 transition-all active:scale-90"
        >
          <ArrowRight size={18} strokeWidth={2.5} />
        </button>

        <div className="text-center">
          <p className="text-xs font-bold text-slate-400">رقم البلاغ</p>
          <p className="text-sm font-black text-slate-800 font-mono">{localIssue.trackingId}</p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={handleShare}
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 hover:bg-emerald-50 text-slate-500 hover:text-emerald-600 transition-all active:scale-90"
          >
            {copied ? <Check size={16} className="text-emerald-500" /> : <Share2 size={16} />}
          </button>
          <button
            onClick={() => setIsVoiceAssistantOpen(true)}
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-emerald-600 text-white shadow-md hover:bg-emerald-700 transition-all active:scale-90"
          >
            <Mic size={16} />
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Hero Image */}
        <div className="relative bg-slate-200 aspect-video w-full overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.img
              key={currentImageIndex}
              ...
              src={(localIssue.mediaUrls && localIssue.mediaUrls.length > 0) ? localIssue.mediaUrls[currentImageIndex] : `https://picsum.photos/seed/${localIssue.id}/800/450`}
              alt="Issue"
          ...
          <div className="absolute top-3 right-3">
            <span className={cn(
              ...
              localIssue.status === 'resolved' ? "bg-emerald-500 text-white" :
              localIssue.status === 'in-progress' ? "bg-amber-500 text-white" :
              "bg-white/90 backdrop-blur-sm text-slate-700"
            )}>
              <span className={cn("w-1.5 h-1.5 rounded-full", localIssue.status === 'resolved' ? "bg-white" : localIssue.status === 'in-progress' ? "bg-white animate-pulse" : "bg-slate-400")} />
              {statusLabel}
            </span>
          </div>
          ...
          {localIssue.mediaUrls && localIssue.mediaUrls.length > 1 && (
            <>
              <button onClick={() => setCurrentImageIndex(prev => prev > 0 ? prev - 1 : localIssue.mediaUrls!.length - 1)}
                className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center backdrop-blur-sm hover:bg-black/60 transition-all">
                <ChevronLeft size={18} />
              </button>
              <button onClick={() => setCurrentImageIndex(prev => prev < localIssue.mediaUrls!.length - 1 ? prev + 1 : 0)}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center backdrop-blur-sm hover:bg-black/60 transition-all">
                <ChevronRight size={18} />
              </button>
              <div className="absolute bottom-3 inset-x-0 flex justify-center gap-1.5">
                {localIssue.mediaUrls.map((_, idx) => (
                  <div key={idx} className={cn("h-1 rounded-full transition-all", idx === currentImageIndex ? "w-5 bg-white" : "w-1.5 bg-white/40")} />
                ))}
              </div>
            </>
          )}
          ...
          <div className="absolute bottom-0 inset-x-0 p-4">
            <div className="flex items-center gap-2 text-white/80 text-xs mb-1">
              <MapPin size={12} />
              <span className="truncate">{localIssue.location.address}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-4 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className={cn(
                ...
                localIssue.severity === 3 ? "bg-rose-50 text-rose-600" : localIssue.severity === 2 ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"
              )}>
                {localIssue.severity === 3 ? '🔴 عاجل' : localIssue.severity === 2 ? '🟡 متوسط' : '🟢 عادي'}
              </div>
              <span className="text-xs font-medium text-slate-400 bg-slate-50 px-2.5 py-1 rounded-lg">{typeLabel}</span>
            </div>
            ...
            <div className="flex items-center justify-between pt-1 border-t border-slate-50">
              <button
                onClick={handleUpvote}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 text-slate-500 transition-all active:scale-95 text-xs font-bold"
              >
                <ThumbsUp size={13} />
                <span>{localIssue.reportCount || 0}</span>
              </button>
              <div className="flex items-center gap-1 text-slate-400 text-xs">
                <Clock size={11} />
                <span>{localIssue.createdAt ? formatTimeAgo((localIssue.createdAt as any).seconds * 1000) : 'الآن'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-emerald-600 rounded-2xl p-4">
              <p className="text-emerald-200 text-[10px] font-bold mb-1">رقم التتبع</p>
              <p className="text-white font-black font-mono text-sm tracking-wider">{localIssue.trackingId}</p>
              <ShieldCheck size={16} className="text-emerald-300 mt-2" />
            </div>

            {localIssue.assignedInstitution ? (
              <div className="bg-white rounded-2xl p-4 border border-slate-100">
                <p className="text-slate-400 text-[10px] font-bold mb-1">الجهة المسؤولة</p>
                <p className="text-slate-800 font-bold text-xs leading-tight">{localIssue.assignedInstitution}</p>
                {INSTITUTIONS.find(i => i.name === localIssue.assignedInstitution)?.website && (
                  <a href={INSTITUTIONS.find(i => i.name === localIssue.assignedInstitution)?.website}
                    target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 text-emerald-600 text-[10px] font-bold mt-2 hover:underline">
                    <ExternalLink size={10} /> الموقع الرسمي
                  </a>
                )}
              </div>
            ) : (
              <div className="bg-white rounded-2xl p-4 border border-dashed border-slate-200 flex items-center justify-center">
                <p className="text-slate-300 text-xs font-medium text-center">لم تُعيَّن جهة بعد</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-400 font-medium bg-slate-50 px-2 py-1 rounded-lg">
                {localIssue.expectedResolutionAt ? `متوقع: ${new Date(localIssue.expectedResolutionAt).toLocaleDateString('ar')}` : 'قيد التقييم'}
              </span>
            </div>

            <div className="relative flex justify-between items-start pt-2">
              {/* Progress line */}
              <div className="absolute top-[18px] left-4 right-4 h-0.5 bg-slate-100 -z-10" />
              <div
                className="absolute top-[18px] right-4 h-0.5 bg-emerald-500 -z-10 transition-all duration-1000"
              ...
              {STAGES.map((s, idx) => {
                ...
                return (
                  <div key={s.id} className="flex flex-col items-center gap-1.5 flex-1">
                    <div className={cn(
                      "w-9 h-9 rounded-xl flex items-center justify-center border-2 transition-all duration-500 z-10 bg-white",
                      isPast ? "bg-emerald-500 border-emerald-400 text-white" :
                      isCurrent ? "bg-emerald-600 border-emerald-500 text-white scale-110 shadow-md shadow-emerald-200" :
                      "border-slate-100 text-slate-300"
                    )}>
                      {isPast ? <Check size={14} strokeWidth={3} /> : React.cloneElement(s.icon as any, { size: 13, strokeWidth: 2.5 })}
                    </div>
                    <span className={cn(
                      "text-[9px] font-bold text-center leading-tight",
                      isCurrent ? "text-emerald-600" : isPast ? "text-emerald-500" : "text-slate-300"
                    )}>{s.label}</span>
                    {relevantMove && (
                      <span className="text-[8px] text-slate-400 font-medium">{formatTimeAgo(relevantMove.timestamp)}</span>
                    )}
                  </div>
                );
              })}
          ...
          {/* Official Actions */}
          {userProfile && (userProfile.role === 'official' || userProfile.role === 'partner') && (
            <div className="bg-white rounded-2xl p-4 border border-slate-100 space-y-4">
              <h4 className="text-xs font-bold text-slate-500 flex items-center gap-2">
                <Building2 size={13} /> أدوات المسؤول
              </h4>

              {userProfile.role === 'official' && !localIssue.assignedInstitution && (
                <div className="space-y-2">
                  <p className="text-xs text-slate-500 font-medium">تعيين الجهة المسؤولة:</p>
                  <div className="space-y-2">
                    {INSTITUTIONS.map(inst => (
                      <button key={inst.id}
                        onClick={() => setConfirmAction({ type: 'assignment', value: inst.name, title: 'تحويل البلاغ', description: `سيتم تحويل البلاغ رسمياً إلى ${inst.name}. هل أنت متأكد؟` })}
                        className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl text-right hover:border-emerald-400 hover:bg-emerald-50 transition-all group flex items-center justify-between"
                      >
                        <ExternalLink size={13} className="text-slate-300 group-hover:text-emerald-500 shrink-0" />
                        <div>
                          <p className="text-sm font-bold text-slate-800 group-hover:text-emerald-700">{inst.name}</p>
                          <p className="text-[10px] text-slate-400">{inst.description}</p>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              {localIssue.assignedInstitution && (
                <div className="flex items-center gap-3 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                  ...
                  <div className="text-right">
                    <p className="text-[10px] text-emerald-600 font-bold">موكل رسمياً إلى</p>
                    <p className="text-sm font-black text-slate-800">{localIssue.assignedInstitution}</p>
                  </div>
                </div>
              )}

              <div className="grid grid-cols-3 gap-2">
                {[{ id: 'verified', label: 'تأكيد' }, { id: 'in-progress', label: 'تنفيذ' }, { id: 'completed', label: 'إكمال' }].map(opt => (
                  <button key={opt.id}
                    onClick={() => setConfirmAction({ type: 'status', value: opt.id as any, title: 'تغيير الحالة', description: `هل ترغب في تغيير حالة البلاغ إلى "${opt.label}"؟` })}
                    className={cn(
                      "py-2.5 rounded-xl text-xs font-bold transition-all border",
                      localIssue.status === opt.id ? "bg-emerald-600 text-white border-emerald-500 shadow-sm" : "bg-slate-50 text-slate-500 border-slate-100 hover:border-emerald-300 hover:text-emerald-600"
                    )}
                  >
                    {opt.label}
                  </button>
          ...
          {/* Citizen Resolve */}
          {auth.currentUser && auth.currentUser.uid === localIssue.reporterId && localIssue.status !== 'resolved' && (
            <button
              onClick={() => setConfirmAction({ type: 'resolve', title: 'اعتماد الحل', description: 'هل تؤكد أن المشكلة قد حُلّت بشكل مرضٍ؟' })}
              className="w-full py-3 bg-white border-2 border-emerald-200 text-emerald-600 rounded-2xl text-sm font-bold hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              <CheckCircle size={16} /> اعتماد الحل وإغلاق البلاغ
            </button>
          )}
          ...
          <div className="bg-white rounded-2xl p-4 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                <History size={15} className="text-slate-400" /> سجل الإجراءات
              </h4>
              <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded-lg">{movements.length} إجراء</span>
            </div>

            {movements.length > 0 ? (
              <div className="relative space-y-4 pr-5 before:absolute before:right-2 before:top-1 before:bottom-1 before:w-px before:bg-slate-100">
                {movements.map((move, idx) => {
                  const inst = INSTITUTIONS.find(i => i.name === move.institutionName);
                  return (
                    <motion.div key={move.id}
                      ...
                      className="relative"
                    >
                      <div className="absolute -right-[17px] top-1.5 w-5 h-5 rounded-full border-2 border-white bg-white flex items-center justify-center shadow overflow-hidden">
                        {inst?.logo ? <img src={inst.logo} className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <div className="w-2 h-2 rounded-full bg-emerald-400" />}
                      </div>
                      <div className="bg-slate-50 rounded-xl p-3 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-slate-400">{move.timestamp ? formatTimeAgo(move.timestamp) : 'الآن'}</span>
                          <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">{move.status}</span>
                        </div>
                        <p className="text-sm text-slate-700 font-medium text-right">{move.description}</p>
                        <p className="text-[10px] text-slate-400 text-right font-medium">{move.institutionName}</p>
                      </div>
                    </motion.div>
                  );
                })}
              ...
              <p className="text-center text-slate-300 text-xs py-6">لا توجد سجلات بعد</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                <MessageSquare size={15} className="text-slate-400" /> النقاش
              </h4>
              <span className="text-[10px] text-slate-400 font-medium">{comments.length} تعليق</span>
            </div>
            ...
            {comments.length > 0 ? (
              <div className="space-y-3">
                {comments.map((c, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-slate-400">{c.createdAt ? formatTimeAgo(c.createdAt.seconds * 1000) : 'الآن'}</span>
                      <div className="flex items-center gap-1.5">
                        <div className="w-5 h-5 bg-slate-200 rounded-full flex items-center justify-center"><Bot size={11} className="text-slate-500" /></div>
                        <span className="text-[10px] font-bold text-slate-600">مواطن #{c.userId.slice(0, 4)}</span>
                      </div>
                    </div>
                    <p className="text-sm text-slate-700 font-medium text-right">{c.text}</p>
                  </motion.div>
                ))}
              </div>
            ) : (
              <p className="text-center text-slate-300 text-xs py-4">لا توجد تعليقات — كن أول من يشارك!</p>
            )}
          </div>
      ...
      {/* Comment Input Bar */}
      <div className="absolute bottom-0 inset-x-0 bg-white/90 backdrop-blur-md border-t border-slate-100 p-3 space-y-2 safe-area-pb">
        {userProfile?.role === 'official' && (
          <div className="flex justify-end">
            <button
              onClick={() => handleAddComment('تم استلام البلاغ وسيجري التعامل معه في أقرب وقت ممكن. نشكركم على تفهمكم.')}
              className="px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full text-[10px] font-bold hover:bg-emerald-600 hover:text-white transition-all"
            >
              رد سريع: تأكيد الاستلام
            </button>
          </div>
        ...
        <div className="flex gap-2">
          <button
            disabled={submitting || !newComment.trim()}
            onClick={() => handleAddComment(newComment)}
            className="w-11 h-11 bg-emerald-600 text-white rounded-xl flex items-center justify-center shadow-md active:scale-90 disabled:opacity-40 transition-all shrink-0 hover:bg-emerald-700"
          >
            <ArrowRight size={18} strokeWidth={2.5} />
          </button>
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-slate-100 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 focus:bg-white transition-all text-right placeholder:text-slate-400 border border-transparent focus:border-emerald-200"
            value={newComment}
            onChange={(e) => setNewComment(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleAddComment(newComment)}
          />
          ...
          <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
            ...
            >
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-slate-500 font-medium">{confirmAction.description}</p>
              </div>
              <div className="space-y-2">
                <button
                  disabled={isProcessing}
                  onClick={async () => {
                    setIsProcessing(true);
                    try {
                      if (confirmAction.type === 'status') await handleStatusUpdate(confirmAction.value);
                      else if (confirmAction.type === 'assignment') await assignInstitution(confirmAction.value);
                      else if (confirmAction.type === 'resolve') await handleResolve();
                      setConfirmAction(null);
                    } catch (err) { console.error(err); }
                    finally { setIsProcessing(false); }
                  }}
                  className="w-full py-3.5 bg-emerald-600 text-white font-bold rounded-2xl text-sm hover:bg-emerald-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95"
                >
                  {isProcessing ? <><Loader2 size={16} className="animate-spin" /> جاري التنفيذ...</> : <><Check size={16} /> تأكيد</>}
                </button>
                <button
                  disabled={isProcessing}
                  onClick={() => setConfirmAction(null)}
                  className="w-full py-3 bg-slate-100 text-slate-500 font-bold rounded-2xl text-sm hover:bg-slate-200 transition-all"
                >
                  إلغاء
                </button>
              </div>
